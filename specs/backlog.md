...
| Feature | Notes |
|---------|-------|
| Custom invite domain | Branded share links (e.g. `https://play.gridly.app/i/{id}`) instead of Supabase project-ref URL. Requires domain purchase plus Supabase custom domain add-on or static host. Verified Universal Links (iOS) and App Links (Android) for one-tap open without a browser landing page; `EXPO_PUBLIC_INVITE_LINK_BASE` env swap; EAS rebuild; 301 redirects from old Supabase invite URLs. [v3.2](./v3.2.md) fixes Android open-from-browser without a custom domain; promote this item when ready to buy a domain. |
